...
# Configure these for the model you're comparing — these are the worksheet/tab names
# present in both files. Use [] to dump every tab.
TABS = ['{{TAB_1}}', '{{TAB_2}}']
...
- Treat numeric values like 1234, 1,234, and 1234.00 as equivalent
- Reserved/unused slots are expected and should not be reported as differences
- Focus on MEANINGFUL differences that affect the model's accuracy
...
The script sanitizes tab names by replacing spaces, periods, and slashes with underscores. So `My Tab Name` becomes `My_Tab_Name.txt`. Use this when writing the comparison agent prompt to point at the right file.
