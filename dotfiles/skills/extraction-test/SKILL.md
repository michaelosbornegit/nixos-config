...
- **Local server running** in Development mode at the configured local URL (dev endpoints are only registered in Development)
- **Auth token**: A valid JWT token must be saved to `/tmp/.api_token` — only needed for Steps 3-5 (create engagement, get doc slots, upload files). Steps 6-7 use dev endpoints that bypass auth.
...
1. Ensure the app is open in Chrome and logged in at the local frontend URL.
   ...
   (async () => {
     ...
     for (const key of msalKeys) {
       try {
         ...
         if (entry.secret) {
           console.log('__API_TOKEN__:' + entry.secret);
           return;
         }
       } catch(e) {}
     }
     console.log('__API_TOKEN__:NOT_FOUND');
   })();
   ```
   Then read console:
   ```
   mcp__claude-in-chrome__read_console_messages  (tabId, pattern: "__API_TOKEN__")
   ```

4. Save and verify:
   ```bash
   echo '<extracted_token>' > /tmp/.api_token
   curl -s -o /dev/null -w "%{http_code}" "$API/engagements" -H "Authorization: Bearer $(cat /tmp/.api_token)"
   # Should return 200
   ```
...
Configure these from your local environment — never hardcode deployed hostnames in this skill:

| Endpoint Type | Base URL | Auth Required |
|---|---|---|
| **Authenticated endpoints** (create, upload, poll) | `<LOCAL_API_BASE>/api` | Yes (JWT) |
| **Dev endpoints** (extract, download) | `<LOCAL_API_BASE>/api/dev` | No |
| **Deployed** | `<DEPLOYED_API_BASE>/api` | Yes (JWT, no dev endpoints) |

Set variables (ask the user for actual values, or read from a local config file like `~/.config/extraction-test.env`):
```bash
API="<LOCAL_API_BASE>/api"
DEV_API="<LOCAL_API_BASE>/api/dev"
...
If creating a fresh test, ask the user what valuationYear to use. The year placeholders in document types create the prior three full years plus YTD. For example:
...
If no reference engagement exists, map files manually based on the document type names defined in your client config (typically a `<org_id>.json` file under the server's client-documents config directory).
...
- `orgId` (string) — must match one of the configured organization IDs in your client-documents config
...
2. **Launch parallel subagents** — one per configured tab. Each agent reads its tab pair from `/tmp/compare/ours/` and `/tmp/compare/answer/` and reports differences.
...
| What | Where to look |
|---|---|
| Dev endpoints (extract, download) | Server `Endpoints/` folder, file ending in `DevAuthEndpoints.cs` (or similar) |
| Engagement endpoints | Server `Endpoints/EngagementEndpoints.cs` |
| Input tab / download endpoints | Server `Endpoints/InputTabEndpoints.cs` |
| Document types config | Server `Configuration/ClientDocuments/<org_id>.json` |
